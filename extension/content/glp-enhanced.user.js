...
`;
    ...
    function getFeatureRegistry() {
        return [
            ...
            { id: 'feed.keywordFilters', routes: ['feed'], init: applyKeywordFilters, apply: applyKeywordFilters, destroy: () => document.querySelectorAll('.glp-keyword-hidden, .glp-keyword-highlight').forEach(node => node.classList.remove('glp-keyword-hidden', 'glp-keyword-highlight')) },
    ...
    function initInfiniteScroll() {
        ...
        const loadNext = async () => {
            ...
            try {
                ...
                if (!resp.ok) { exhausted = true; return; }
    ...
    function getThreadId(row) {
        ...
        if (!link) return null;
        const match = link.href.match(/message(\d+)/);
    ...
    function initHideThreadButtons() {
        ...
        document.querySelectorAll('.threads tbody tr:not(.threads_header_row)').forEach(row => {
            ...
            btn.addEventListener('click', (e) => {
                ...
                e.stopPropagation();
                const threadId = getThreadId(row);
                if (threadId) hideThread(threadId);
            });
            td.appendChild(btn);
            row.appendChild(td);
        });
    ...
    function applyKeywordFilters() {
        ...
        if (highlightWords.length === 0 && hideWords.length === 0) return;

        document.querySelectorAll('.threads .sfr a').forEach(link => {
            ...
            // Highlight
            if (highlightWords.some(w => title.includes(w)) && !link.dataset.glpHighlighted) {
    ...
    function showLightbox(src) {
        ...
        document.addEventListener('keydown', function galKey(e) {
            if (!document.getElementById('glp-lightbox')) { document.removeEventListener('keydown', galKey); return; }
            if (e.key === 'Escape') { overlay.remove(); document.removeEventListener('keydown', galKey); }
        });
    ...
    function initThreadPreview() {
        ...
        document.addEventListener('mouseover', (e) => {
            ...
            previewTimeout = setTimeout(async () => {
                ...
                title.textContent = link.textContent.trim().substring(0, 100);
    ...
    function initQuickSearch() {
        ...
        input.addEventListener('input', () => {
            ...
            if (query.length < 2) { counter.textContent = ''; searchMatches = []; return; }
        ...
        input.addEventListener('keydown', (e) => {
            if (e.key === 'Enter') {
                ...
                if (e.shiftKey) searchCurrentIdx = (searchCurrentIdx - 1 + searchMatches.length) % searchMatches.length;
                else searchCurrentIdx = (searchCurrentIdx + 1) % searchMatches.length;
                updateSearchCurrent();
